...
#pode ser util(salvando o model em json e yaml) https://machinelearningmastery.com/save-load-keras-deep-learning-models/
...
model.fit(x, y, epochs=50, validation_split=0.001, callbacks=[early_stopping_monitor])


yteste = model.predict(xteste)
for i in range(len(xteste)):
    print("X=%s, Predicted=%s" % (xteste[i], yteste[i]))
